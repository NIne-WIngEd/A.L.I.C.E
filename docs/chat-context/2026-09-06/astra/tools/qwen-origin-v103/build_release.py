...
PACKAGE=ROOT/'package/ALICE_MC10D_QWEN_ORIGIN_REVISION_v1.0.3'
OUTPUT=ROOT/'dist'
...
launcher=OUTPUT/'Start-ALICEAstraQwenQualificationV103.ps1'
text=(ROOT/'Start-ALICEAstraQwenQualificationV103.template.ps1').read_text(encoding='utf-8').replace('__ZIP_SHA256__',sha(archive).upper())
...
with tempfile.TemporaryDirectory(prefix='alice-qwen-release-') as temp:
    ...
    receipt.update({'revision_id':rules['revision_id'],'superseded_package_sha256':rules['old_package_sha256'],
                    ...
                    'local_state_fixture_scope':'Synthetic local timestamps; production requires the exact observed state hash',
                    'live_scheduler_and_tls_scope':'Actual uploaded diagnostic observation; revision and job behavior tested offline; live v103 pending'})
