...
$Expected = "05E1E8AA3FA1233D8ACB25DA5E87A93B211791F5E568DB850F83DCA3499630F2"
$ZipBase = "ALICE_MC10D_QWEN_ORIGIN_REVISION_v1.0.3"
$PackageName = $ZipBase
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ZipPath)) { throw "The exact v1.0.3 ZIP is missing beside this launcher, or its SHA-256 differs." }
...
'@
    ...
    Write-Host "Verified a2 source-origin repair. Preserve prior state, record the package revision, then continue the same approved public Qwen calibration: Magnolia CPU, 20 CPUs, 48 GiB, six-hour limit."
    ...
    Write-Host "ALICE_ASTRA_QWEN_V103_EXIT=$Code"
    ...
    Write-Host "ALICE_ASTRA_QWEN_V103_STOP: $($_.Exception.Message)" -ForegroundColor Yellow
