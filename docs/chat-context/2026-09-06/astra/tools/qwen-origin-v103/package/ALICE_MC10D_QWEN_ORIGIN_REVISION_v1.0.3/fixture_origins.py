...
def local_prepared(vault):
    """Synthetic local timestamps; actual trace supplies a hash, not raw state bytes."""
    path = Path(vault) / 'tools/alice-astra/qwen-fallback-a2/controller-state.json'
    rules = dict(revision.policy())
    c.write(path, {'schema': 'alice.mc10d.qwen.controller-state.v1', 'run_id': c.RUN_ID,
                  'package_sha256': rules['old_package_sha256'], 'amendment_sha256': c.APPROVED_SHA,
                  'phase': 'PREPARED', 'staged': True, 'created_at': 'SYNTHETIC_TEST_TIMESTAMP'})
    rules['old_local_state_sha256'] = c.file_sha(path)
    return path, rules
