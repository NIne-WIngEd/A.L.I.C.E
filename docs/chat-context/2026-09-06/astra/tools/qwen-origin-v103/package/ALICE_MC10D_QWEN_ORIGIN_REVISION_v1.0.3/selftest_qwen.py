...
def simulation(root, *, wrong_task=None, truncated_task=None, slow=False, telemetry_fail=False, package_sha=FAKE_PACKAGE):
    ...
    calls=[]
    fixture_origins.completed_revision(run,package_sha)
    rules=infra_recovery.policy()
    trust={'schema':'fixture-tls','ca_path':'fixture-only','ca_sha256':'4'*64,'ca_bytes':1,'verify_mode':'CERT_REQUIRED','check_hostname':True,'openssl':'fixture'}
    c.write(run/'tls-trust.json',trust)
    c.write(run/'parent-reconciliation.json',{'source_run_id':c.CALIBRATION_ID,'source_job_id':'575089','source_exit_code':'76:0','zero_inference_confirmed':True,'source_run_modified':False,'source_result_zip_sha256':rules['source_result_zip_sha256'],'source_result_sha256':rules['source_critical_files']['result.json']})
    parent=c.read(run/'parent-reconciliation.json')
    parent.update(source_package_manifest_sha256=rules['source_critical_files']['package-manifest.json'],
        source_package_sha256=rules['source_workload_sha256'], source_manifest_origin='PACKAGE',
        source_package_origin=evidence_origins.source_package(rules).as_posix(), source_queue_query_succeeded=True)
    c.write(run/'parent-reconciliation.json',parent)
    c.write(run/'infrastructure-preflight.json',{'status':'PASS','execution_run_id':c.RUN_ID,'parent_reconciliation_sha256':c.file_sha(run/'parent-reconciliation.json'),'tls_trust_sha256':c.file_sha(run/'tls-trust.json'),'ca_sha256':trust['ca_sha256'],'runtime_https_head_status':200,'model_manifest_sha256':digest})
    ...
    def fake_process(argv,**kwargs):
        ...
        key=argv[1];payload=tuple(Path(x).read_bytes() for x in argv[4+1:])
        if key in immutable_ledger and immutable_ledger[key]!=payload:
            return subprocess.CompletedProcess(argv,73,b'RUN_ID_COLLISION=true',b'')
        immutable_ledger[key]=payload
        return subprocess.CompletedProcess(argv,1 if telemetry_fail and argv[3]=='COMPLETED' else 0,b'',b'')
    with mock.patch.object(c,'ROOT',root),mock.patch.object(c,'REMOTE_RUN',run),mock.patch.dict(os.environ,{'SLURM_JOB_ID':'12345','SLURM_CPUS_PER_TASK':'20','SLURM_JOB_PARTITION':'node','SLURM_MEM_PER_NODE':'49152','SLURM_JOB_GPUS':''}),mock.patch.object(worker.os,'sched_getaffinity',return_value=set(range(20)),create=True),mock.patch.object(secure_https,'configure',return_value=trust),mock.patch.object(worker,'prepare_runtime',side_effect=runtime),mock.patch.object(worker,'prepare_model',side_effect=prepare_model),mock.patch.object(worker,'start_service',return_value=(proc,'http://fixture.invalid',policy['version'])),mock.patch.object(worker,'api',side_effect=api),mock.patch.object(worker,'stream_request',side_effect=stream),mock.patch.object(worker.subprocess,'run',side_effect=fake_process),quiet():
        code=worker.work(run,package_sha)
    c.write(run/'run.json',package_revision.descriptor(package_sha))
    latest=c.read(run/'telemetry-publication.json')['snapshot_id']
    for event in (run/'telemetry-events').iterdir():
        if event.name!=latest:shutil.rmtree(event)  # Only temporary simulation fixtures are pruned.
...
class LifecycleTests(unittest.TestCase):
    ...
    def test_terminal_telemetry_can_recover_without_inference(self):
        ...
        with mock.patch.object(remote.subprocess,'run',return_value=subprocess.CompletedProcess([],0,b'',b'')) as push:
...
class TransportTests(unittest.TestCase):
    ...
    def _submission(self,lose_response,discover_after):
        run=self.root/'run'
        fixture_origins.completed_revision(run,FAKE_PACKAGE)
        ...
        with mock.patch.object(c,'ROOT',self.root),mock.patch.object(c,'REMOTE_RUN',run),mock.patch.object(remote,'submission_lock',controller.controller_lock),mock.patch.object(remote,'find_submitted',side_effect=lambda:jobs[0] if jobs else None),mock.patch.object(remote,'cmd',side_effect=command),mock.patch.object(remote.shutil,'which',return_value='/fixture/tool'),mock.patch.object(infra_recovery,'preflight'):
            if lose_response:
...
class ControllerTests(unittest.TestCase):
    def test_connection_loss_then_collection_then_publication_retry_reuses_evidence(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            _,revision_rules=fixture_origins.local_prepared(root/'vault')
            revision_patch=mock.patch.object(package_revision,'policy',return_value=revision_rules)
            revision_patch.start();self.addCleanup(revision_patch.stop)
            ...
            class FakeMagnolia:
                ...
                def action(self,action):
                    ...
                    if action=='revise':return package_revision.receipt(package_sha)
            ...
            state=c.read(args.vault_root/'tools/alice-astra/qwen-fallback-a2/controller-state.json')
