...
$Expected = "EDC325F70513CFE6E0E27289BA05132E2DEFBFDF306DCAFD0550740955293647"
$ZipBase = "ALICE_MC10D_QWEN_PROVIDER_REVISION_v1.0.4"
$PackageName = $ZipBase
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ZipPath)) { throw "The exact v1.0.4 ZIP is missing beside this launcher, or its SHA-256 differs." }
    Write-Host "package_sha256=$Expected"
    $script:Python = Resolve-Python
    Write-Host "PYTHON=$script:Python"
    $RunId = [Guid]::NewGuid().ToString("N").Substring(0,12)
    $RunRoot = Join-Path $OutputRoot "ALICE_QwenA2_RUN_$RunId"
...
'@
    ...
    Write-Host "Verified observed scheduler repair. Preserve the unfinished v103 intent, record the explicit v104 revision, then continue the same approved public Qwen calibration: Magnolia CPU, 20 CPUs, 48 GiB, six-hour limit."
    ...
    Write-Host "ALICE_ASTRA_QWEN_V104_EXIT=$Code"
    ...
    Write-Host "ALICE_ASTRA_QWEN_V104_STOP: $($_.Exception.Message)" -ForegroundColor Yellow
