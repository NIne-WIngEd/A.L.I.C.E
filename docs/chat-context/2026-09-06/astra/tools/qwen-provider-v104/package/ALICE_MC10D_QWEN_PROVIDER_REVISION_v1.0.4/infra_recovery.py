...
def reconcile_source(command):
    ...
    c.require(not any(row[0] == job or row[1] == c.CALIBRATION_ID for row in active), 'Source job is still queued or running')
    rows = [row for row in scheduler.historical_rows(command, job) if row[0] == job]
    c.require(len(rows) == 1, 'Source accounting receipt missing or ambiguous')
    row = rows[0]
