...
def publish_stored(run, folder):
    ...
    markers = []
    try:
        result = subprocess.run([str(c.ROOT / 'bin/rayan-telemetry-push.sh'), item['snapshot_id'], STAGE, item['status'], item['slurm_job_id'], str(folder / 'application.log'), str(folder / 'manifest.json'), str(folder / 'receipt.json')], capture_output=True, timeout=90)
        code = result.returncode
        diagnostic = (result.stdout + result.stderr).decode('utf-8', 'replace')
        # Deliberately capture known operational markers, never arbitrary command output.
        markers = [x for x in ('RUN_ID_COLLISION', 'LEDGER_COLLISION', 'Permission denied', 'Could not resolve', 'non-fast-forward', 'Connection timed out') if x in diagnostic]
    except (OSError, subprocess.TimeoutExpired) as exc:
        code = 75
        markers = [type(exc).__name__]
