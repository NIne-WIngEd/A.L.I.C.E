...
PACKAGE=ROOT/'package/ALICE_MC10D_QWEN_PUBLIC_QUALIFICATION_v1.0.0'
OUTPUT=ROOT/'dist'
...
launcher=OUTPUT/'Start-ALICEAstraQwenQualificationV100.ps1'
text=(ROOT/'Start-ALICEAstraQwenQualificationV100.template.ps1').read_text(encoding='utf-8').replace('__ZIP_SHA256__',sha(archive).upper())
...
with tempfile.TemporaryDirectory(prefix='alice-qwen-release-') as temp:
    ...
    tests=subprocess.run([sys.executable,str(package/'selftest_qwen.py')],capture_output=True,text=True,timeout=120)
