...
class Magnolia:
    def __init__(self,key,package_sha,diagnostics=None):
        ...
        self.directory=str(c.ROOT/'packages'/c.RUN_ID)
    ...
    def download(self,receipt,path):
        c.require(receipt['remote_path']==str(c.REMOTE_RUN/'result-bundle.zip'),'unexpected remote result path')
...
def execute(args):
    ...
    state_root=args.vault_root/'tools/alice-astra/qwen-fallback-a1'
    ...
    with controller_lock(state_root/'controller.lock'):
        ...
        if state_path.exists():
            ...
            c.require(state['run_id']==c.RUN_ID and state['package_sha256']==package_sha,'existing approved run uses different package; no fresh run inferred')
        else:
        ...
        print('live_telemetry=https://github.com/NIne-WIngEd/Rayan-Compute-Ledger/tree/main/runs/'+c.RUN_ID,flush=True)
