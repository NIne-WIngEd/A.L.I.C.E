...
def submit(run, package_sha):
    ...
    with submission_lock(run/'submission.lock'):
        descriptor={'run_id':c.RUN_ID,'package_sha256':package_sha,'amendment_sha256':c.APPROVED_SHA,'approved_profile_id':'qwen38_thinking_on_public_calibration_v1'}
...
def evidence_paths(run):
    ...
    for name in ('runtime-decoder.json','zstandard-install.json'):
    ...
    for name in ('tasks.json','draft.json','approval.json','approved.json','runtime_policy.json','source_binding_policy.json'):
        paths['authority/'+name]=c.BASE/'authority'/name
    paths['package-manifest.json']=c.BASE/'PACKAGE_MANIFEST.json'
    return paths
...
def repair_terminal_telemetry(run,state):
    ...
    receipt=c.read(run/'telemetry-receipt.json') if (run/'telemetry-receipt.json').exists() else {}
    recovered=c.read(run/'telemetry-recovery.json') if (run/'telemetry-recovery.json').exists() else {}
    if (publication.get('exit_code')==0 or recovered.get('exit_code')==0) and receipt.get('status') in ('COMPLETED','FAILED'):
        return
    ...
    succeeded=finished.get('result_sha256')==c.file_sha(run/'result.json') if (run/'result.json').exists() else False
    succeeded=succeeded and result.get('qualification_passed') is True
    ...
    if not (run/'telemetry-manifest.json').exists():
        c.write(run/'telemetry-manifest.json',{'schema':'rayan.compute.run-manifest.v1','run_id':c.RUN_ID,'stage':'MC10D_QWEN_PUBLIC_CALIBRATION_V1','provider':'magnolia','slurm_job_id':state['job_id'],'opaque_workload_sha256':descriptor['package_sha256']})
    with (run/'application.log').open('a',encoding='utf-8') as out:out.write(c.canonical(receipt))
    try:
        process=subprocess.run([str(c.ROOT/'bin/rayan-telemetry-push.sh'),c.RUN_ID,'MC10D_QWEN_PUBLIC_CALIBRATION_V1',status,state['job_id'],str(run/'application.log'),str(run/'telemetry-manifest.json'),str(run/'telemetry-receipt.json')],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,timeout=90)
        code=process.returncode
    except (OSError,subprocess.TimeoutExpired):code=75
    c.write(run/'telemetry-recovery.json',{'exit_code':code,'scheduler_terminal_verified':True,'job_id':state['job_id'],'result_sha256':c.file_sha(run/'result.json') if result else None,'raw_model_responses_published':False,'observed_at':c.now()})
...
def main():
    ...
    run=c.REMOTE_RUN
    try:
        if args.action=='submit':obj=submit(run,args.package_sha)
        else:
