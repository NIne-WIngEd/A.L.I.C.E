...
def verify_source_files(source):
    ...
    for name, expected in rules['source_critical_files'].items():
        file = source / name
        c.require(file.is_file() and not file.is_symlink() and c.file_sha(file) == expected, 'Source failure evidence differs: ' + name)
...
def reconcile_source(command):
    ...
    account = command(['sacct', '-X', '-n', '-P', '-j', job, '--format', 'JobIDRaw,JobName%100,State,ExitCode'])
