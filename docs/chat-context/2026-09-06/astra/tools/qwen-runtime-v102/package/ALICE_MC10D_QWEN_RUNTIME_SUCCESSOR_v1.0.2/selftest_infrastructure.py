...
class InfrastructureTests(unittest.TestCase):
    ...
    def source(self):
        folder = self.root / 'runs' / c.CALIBRATION_ID
        folder.mkdir(parents=True)
        archive = c.BASE / 'source/ALICE_QWEN_PUBLIC_RESULT_c926d9e355dd.zip'
        self.assertEqual(c.file_sha(archive), infra.policy()['source_result_zip_sha256'])
