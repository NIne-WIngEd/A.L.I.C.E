...
def cmd(argv, check=True):
    return scheduler.command(argv, check=check)
...
def find_submitted():
    return scheduler.find_submitted(cmd)
...
def submit(run, package_sha):
    ...
    with submission_lock(run/'submission.lock'):
        descriptor=infra_recovery.descriptor(package_sha)
...
def status(run):
    ...
    state,in_queue=scheduler.job_state(cmd,job,c.RUN_ID)
...
def evidence_paths(run):
    ...
    for name in ('runtime-decoder.json','zstandard-install.json','tls-trust.json','parent-reconciliation.json','infrastructure-preflight.json','infrastructure-preflight-failure.json','telemetry-events.jsonl','scheduler-commands.jsonl','telemetry-latest/manifest.json','telemetry-latest/receipt.json','telemetry-latest/application.log'):
    ...
    paths.update(evidence_origins.package_projections(c.BASE,
        evidence_origins.SOURCE_AUTHORITIES + ('infra_recovery.json','request_hashes_v100.json')))
    paths.update(infra_recovery.evidence_paths(run))
    paths.update(telemetry_snapshots.journal_evidence(run))
    return paths
...
def main():
    ...
    try:
        ...
        print(c.canonical({'state':'STOP','run_id':c.RUN_ID,'action':args.action,'error_class':type(exc).__name__,'message':str(exc)[:1200],'last_scheduler_command':scheduler.LAST}),end='')
        return 76
