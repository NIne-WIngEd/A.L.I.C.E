...
import secure_https
from telemetry_snapshots import Telemetry
...
def download(url, target, expected_sha, expected_size, deadline):
    ...
    with secure_https.open_url(url, timeout=60) as source, temp.open('xb') as out:
        size = 0
        while True:
...
def prepare_runtime(run, policy, deadline):
    ...
    try:
        ...
        command([sys.executable,'-m','pip','--isolated','install','--index-url','https://pypi.org/simple','--cert',os.environ['SSL_CERT_FILE'],'--no-deps','--only-binary=:all:','--disable-pip-version-check','--no-input','--target',deps,'--report',run/'zstandard-install.json','zstandard==0.25.0'], timeout=min(300,max(1,deadline-time.monotonic())), log=run/'runtime-extraction.log')
...
def resolve_manifest(run):
    ...
    cached = (run/'model-lock.json').exists() or (run/'model-manifest.json').exists()
    if cached:
        c.require((run/'model-lock.json').is_file() and (run/'model-manifest.json').is_file(), 'Incomplete frozen model manifest')
        lock = c.read(run/'model-lock.json')
        raw = (run/'model-manifest.json').read_bytes()
        c.require(lock['source_url']==url and lock['model_tag']==c.TAG and lock['approved_amendment_sha256']==c.APPROVED_SHA and lock['inference_started'] is False, 'Frozen model lock identity')
        c.require(lock['full_manifest_digest']==c.sha(raw) and lock['manifest_sha256']==c.sha(raw), 'Frozen model manifest changed')
        header = lock.get('registry_header_digest')
    else:
        req = urllib.request.Request(url, headers={'Accept':'application/vnd.docker.distribution.manifest.v2+json'})
        with secure_https.open_url(req, timeout=25) as response:
            raw = response.read(1024*1024+1)
            c.require(len(raw)<=1024*1024, 'Model manifest size exceeds limit')
