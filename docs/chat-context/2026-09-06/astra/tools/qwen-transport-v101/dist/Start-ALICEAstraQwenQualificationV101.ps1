...
$Expected = "AE20351156CDB9DFF4CA7674D2FEE607C9EB4D3D880DFBB5FFD6D23393047575"
$ZipBase = "ALICE_MC10D_QWEN_WINDOWS_TRANSPORT_REPAIR_v1.0.1"
$PackageName = $ZipBase
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ZipPath)) { throw "The exact v1.0.1 transport-repair ZIP is missing beside this launcher, or its SHA-256 differs." }
    Write-Host "repair_package_sha256=$Expected"
    $script:Python = Resolve-Python
    Write-Host "PYTHON=$script:Python"
    $RunId = [Guid]::NewGuid().ToString("N").Substring(0,12)
    $RunRoot = Join-Path $OutputRoot "ALICE_QwenTransportA1_RUN_$RunId"
...
'@
    ...
    Invoke-Python -Arguments @((Join-Path $PackageRoot "selftest_transport.py"))
    if ($script:LastPythonExitCode -ne 0) { throw "Offline selftest failed. No source evidence was changed." }
    Write-Host "Windows remote-path repair verified. Reusing the exact approved v100 workload and the existing run ID."
    Invoke-Python -Arguments @((Join-Path $PackageRoot "runner.py"),"--vault-root",$VaultRoot,"--output-root",$OutputRoot,"--repo-root",$RepoRoot)
    $Code = $script:LastPythonExitCode
    Write-Host "ALICE_ASTRA_QWEN_V101_EXIT=$Code"
    ...
    Write-Host "ALICE_ASTRA_QWEN_V101_STOP: $($_.Exception.Message)" -ForegroundColor Yellow
