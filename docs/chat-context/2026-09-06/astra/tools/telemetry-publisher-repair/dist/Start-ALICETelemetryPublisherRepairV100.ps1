...
$Expected = "9C7DCC00E29F4FAFD1EAA1B57F108DEE23D9538ACC7F4A9940F2A51EB2057D83"
$ZipBase = "ALICE_TELEMETRY_PUBLISHER_REPAIR_v1.0.0"
$PackageName = $ZipBase
...
try {
    ...
    if ([string]::IsNullOrWhiteSpace($ZipPath)) { throw "The exact publisher repair v1.0.0 ZIP is missing beside this launcher, or its SHA-256 differs." }
    ...
    $RunRoot = Join-Path $OutputRoot "ALICE_PublisherRepair_$ReceiptToken"
...
'@
    ...
    Write-Host "The publisher repair is checked on Magnolia before installation. Then it verifies the existing failed a2 terminal snapshot and an identical retry. No inference or new job."
