...
## Semantic probability control precision (job 576168)

On clean Magnolia source `8b2c2488`, CPU bfloat16 preflight job `576168` FAILED `1:0` after 15 seconds in `SemanticOperatorState.validate`: `relation/stop/unknown masses violate recurrent survival conservation`. The failure arose before the graph and Executor. The earlier CPU job `576167` reached a later Executor scatter, showing that the operator failure depends on half-precision rounding and initial states rather than proving that the Executor repair failed.

The active semantic operator initialized recurrent survival in the autocast-produced state dtype and formed event and candidate distributions at autocast precision, yet validates survival conservation at `1e-5`. Its learned projections remain under autocast; the validated candidate/event probabilities, survival recurrence, coverage, and applicability control outputs now use float32. A standalone CPU bfloat16 test checks multiple random initializations and step depths, and the full-stack autocast test checks conservation before graph/Executor assertions. The unchanged structural adapter consumes these probability tensors and retains source gradients. This is a numerical interface correction across the active operator/control path; no topology, public curriculum, threshold, private gradient, or FINAL change follows.

Local Python syntax and diff checks pass. Neither the new CPU tests nor a two-GPU fp16 route has been observed on this repaired source. Keep job `576168` and older artifacts. On a clean exact-head Magnolia checkout, run the two named CPU autocast tests together via the validated one-session udocker route. If both pass, repeat source-bound P42 into a fresh CPU root, then P39PN into a fresh mixture root, then P43 into a fresh GPU root. Inspect each receipt before opening the next gate.
