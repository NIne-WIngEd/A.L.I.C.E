...
## Fictional contract fixture progress (2026-09-26)

`training/fbm/fixtures/fictional_case_shape_v0.1.jsonl` now contains seven linked, manually authored cases for Mira and Theo: attributed outside quote, scoped host request, correction/dependency invalidation, sparse second-person assembly, hypothetical rehearsal, a conflicting later *scripted fictional* follow-up, and cross-person evidence exclusion. Each row is explicitly `training_eligible=false` and `evaluation_eligible=false`; the follow-up is marked `fictional_scripted_event`, not independently observed experience. The fixture demonstrates the case fields and subject boundaries. It supplies no learned builder competence, independent real outcomes, truly independent generator families, sealed transfer holdouts, or consumer qualification. The next data gate remains genuinely authorized input/target/outcome cases and frozen family-held-out checks under the rights and authority rules above.
