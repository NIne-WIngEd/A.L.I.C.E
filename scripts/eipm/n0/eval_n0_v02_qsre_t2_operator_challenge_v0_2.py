...
def main() -> None:
    ...
    operator = QSRET2OperatorEncoder(
        QSRET2OperatorConfig(**contract["operator_model"])
    )
    operator.load_state_dict(
        torch.load(operator_checkpoint, map_location="cpu"),
        strict=True,
    )
    ...
    payload = {
        "schema": "alice.eipm.n0.qsre-t2-locked-operator-challenge-result.v0.2",
