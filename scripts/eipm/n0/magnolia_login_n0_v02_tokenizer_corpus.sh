...
OUT="${N0_V02_TOKENIZER_CORPUS_DIR:-$WORKDIR/tokenizer-corpus-v0.2}"
TARGET_CHARS="${N0_V02_TOKENIZER_CORPUS_CHARS:-120000000}"
MAX_CHUNK_CHARS="${N0_V02_MAX_DOCUMENT_CHUNK_CHARS:-16000}"
...
echo "===== N0 V0.2.1 BALANCED TOKENIZER CORPUS ====="
date -Is
echo "repo_root=$ROOT"
echo "source_config=$SOURCE_CONFIG"
echo "output=$OUT"
echo "target_total_chars=$TARGET_CHARS"
echo "max_document_chunk_chars=$MAX_CHUNK_CHARS"
...
bash "$ROOT/scripts/eipm/n0/magnolia_udocker_exec.sh" -lc \
  "python scripts/eipm/n0/materialize_public_corpus_v02.py \
    ...
    --max-document-chunk-chars '$MAX_CHUNK_CHARS' \
    --shard-mb 64"

echo "===== N0 V0.2.1 BALANCED TOKENIZER CORPUS COMPLETE ====="
date -Is
