...
from alice_personality.n0.evidence_graph_query_edge_competitive_router import (
    CompetitiveQueryEdgeEvidenceGraphEncoder,
)
from alice_personality.n0.query_edge_competitive_router_bridge import (
...
SCHEMA = "alice.eipm.n0.query-edge-competitive-router-qualification.v0.1"
EXPECTED_FAILED_RESULT_SHA256 = (
    "99fc21394bdfbb8e3500bf0efa7c3ce836a2e071b8b11a1c733f73aa99b6fea2"
)
...
def parent_state_exact(
    ...
    candidate: CompetitiveQueryEdgeEvidenceGraphEncoder,
) -> bool:
...
def make_models(
    *,
    ...
    map_path: Path,
    device: torch.device,
) -> tuple[DualEndpointEvidenceGraphEncoder, CompetitiveQueryEdgeEvidenceGraphEncoder]:
    ...
    bridge = CompetitiveQueryEdgeBridge.from_layer_map_file(
    ...
    if any(
        not name.startswith("competitive_query_edge_bridge.")
        for name in missing
    ) or unexpected:
        raise SystemExit(
...
def main() -> None:
    ...
    for path in (
        map_path, graph_path, failed_path,
        curriculum_path, manifest_path, anchors_path,
    ):
        if not path.is_file():
            raise SystemExit(f"missing competitive-router qualification input: {path}")
    if output.exists():
        raise SystemExit("refusing to overwrite competitive-router qualification")
    ...
    for path, expected in expected_hashes.items():
        if sha256(path) != expected:
            raise SystemExit(f"qualification lineage drift: {path.name}")
    ...
    if not future_trainable_names or any(
        ...
        raise SystemExit("future trainable scope escapes competitive bridge")
    ...
    if not torch.equal(parent_out["field_weights"], candidate_out["field_weights"]):
        raise SystemExit("zero-residual competitive bridge lost exact parent weights")
    if not torch.equal(parent_out["pooled_state"], candidate_out["pooled_state"]):
        raise SystemExit("zero-residual competitive bridge lost exact parent pooled state")
    if not parent_state_exact(parent, candidate):
        raise SystemExit("parent tensors changed during qualification")

    source_proposal = candidate_out["source_residual_proposal"]
    target_proposal = candidate_out["target_residual_proposal"]
    source_residual = candidate_out["source_residual"]
    target_residual = candidate_out["target_residual"]
    if not torch.equal(source_proposal, torch.zeros_like(source_proposal)):
        raise SystemExit("source residual readout is not zero initialized")
    if not torch.equal(target_proposal, torch.zeros_like(target_proposal)):
        raise SystemExit("target residual readout is not zero initialized")
    if not torch.equal(source_residual, torch.zeros_like(source_residual)):
        raise SystemExit("source runtime residual is not zero at initialization")
    if not torch.equal(target_residual, torch.zeros_like(target_residual)):
        raise SystemExit("target runtime residual is not zero at initialization")

    route_probs = candidate_out["route_probabilities_with_noop"]
    if route_probs.shape != (batch, edges + 1):
        raise SystemExit("competitive route shape drift")
    if not torch.allclose(
        ...
        raise SystemExit("competitive route probabilities do not sum to one")
    if not bool((candidate_out["noop_route_probability"] > 0).all().item()):
        raise SystemExit("parent/no-op route missing probability mass")

    # Conflict and PAD are not directional specialist routes.
    edge_probs = candidate_out["edge_route_probability"]
    if not torch.equal(edge_probs[:, 4], torch.zeros_like(edge_probs[:, 4])):
        raise SystemExit("conflict edge received competitive route probability")
    if not torch.equal(edge_probs[:, 5], torch.zeros_like(edge_probs[:, 5])):
        raise SystemExit("PAD edge received competitive route probability")
    ...
    # Non-persisted route-control probe.  Give every edge the same non-zero
    # proposal and prove the exact route probabilities become the exact runtime
    ...
    with torch.no_grad():
        probe.competitive_query_edge_bridge.source_residual_read.bias.fill_(1.0)
        probe.competitive_query_edge_bridge.target_residual_read.bias.fill_(-1.0)
    probe.eval()
    with torch.inference_mode():
        probe_out = probe(
            field_states=field_states,
    ...
    for key, value in required.items():
        if report.get(key) != value:
            raise SystemExit(f"competitive router report drift: {key}")

    receipt: dict[str, Any] = {
        "schema": SCHEMA,
        "status": "PASS_COMPETITIVE_EDGE_ROUTER_NO_GRADIENT_RUNTIME_CONTRACT",
        ...
        "future_trainable_scope": "competitive_query_edge_bridge_only",
        ...
        "explicit_parent_noop_route": True,
        "all_active_directed_edges_compete": True,
        ...
        "n0_complete": False,
        "next_action": (
            "review competitive route-control qualification and define one "
            "separate training-objective decision before any optimizer or GPU"
        ),
    }
    output.write_text(
