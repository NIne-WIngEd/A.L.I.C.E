...
for required in "$CHALLENGE" "$MANIFEST" "$FREEZE_RECEIPT" "$SPEC" "$EVALUATOR" "$LATENT_CONFIG" "$CANDIDATE" "$SEMANTIC_CONFIG" "$SEMANTIC_CHECKPOINT/alice_n0_v02.safetensors" "$TOKENIZER_DIR/tokenizer.json" "$STRUCTURED_CONFIG" "$STRUCTURED_CHECKPOINT/structured_state.safetensors" "$EVIDENCE_ADAPTER" "$EVIDENCE_GRAPH" "$FUSION_CHECKPOINT/cross_context_fusion.safetensors" "$FUSION_CONFIG" "$FUSION_RATIFICATION"; do
  if [[ ! -f "$required" ]]; then
    echo "Missing latent v0.2 frozen challenge artifact: $required" >&2
    exit 2
  fi
done

if [[ -e "$RESULT" ]]; then
  echo "Refusing to overwrite latent v0.2 frozen challenge result: $RESULT" >&2
  exit 3
fi
...
if receipt.get('status')!='FROZEN_CONFIRMATORY_BEFORE_EVALUATION': raise SystemExit('latent v0.2 freeze receipt status drift')
if receipt.get('git_revision')!=head: raise SystemExit(f"latent v0.2 freeze receipt stale: receipt={receipt.get('git_revision')} current={head}")
for path,key in [(challenge_path,'challenge_sha256'),(manifest_path,'manifest_sha256'),(spec_path,'spec_sha256'),(evaluator_path,'evaluator_sha256'),(candidate_path,'candidate_latent_pool_sha256')]:
    if receipt.get(key)!=sha(path): raise SystemExit(f'latent v0.2 frozen artifact drift: {key}')
if receipt.get('candidate_preselected_before_challenge') is not True or receipt.get('checkpoint_selection_on_challenge_forbidden') is not True: raise SystemExit('latent v0.2 confirmatory selection contract drift')
if receipt.get('candidate_weights_changed_after_v0_1') is not False: raise SystemExit('latent v0.2 candidate weights changed after v0.1')
if receipt.get('counterfactual_intervention_point')!='before_parent_cache_and_before_cross_context_fusion': raise SystemExit('latent v0.2 intervention contract drift')
if receipt.get('challenge_rows_used_for_training') is not False or receipt.get('gradient_performed') is not False: raise SystemExit('latent v0.2 governance drift')
print('adaptive_latent_pool_v02_frozen_same_revision_gate=true')
...
echo "adaptive_latent_pool_v02_frozen_challenge_complete=true"
echo "result=$RESULT"
