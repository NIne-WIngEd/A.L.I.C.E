...
OUT_ROOT="$WORKDIR/cross-context-fusion-curriculum-v0.1"
...
mkdir -p "$OUT_ROOT"
python "$ROOT/scripts/eipm/n0/build_n0_v02_cross_context_fusion_curriculum.py" \
  --output "$CURRICULUM" \
  --manifest "$MANIFEST"
...
if manifest.get("schema") != "alice.eipm.n0.v02-cross-context-fusion-curriculum.v0.1":
