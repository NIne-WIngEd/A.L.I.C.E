...
if [[ "$HEAD" != "$EXPECTED" ]]; then
  echo "STOP: final N0 source revision drift HEAD=$HEAD expected=$EXPECTED" >&2
  exit 90
fi
...
FINAL_ROOT="$WORKDIR/qsre-production-core-v1-final-v3"
...
if [[ -e "$FINAL_ROOT" ]]; then
  echo "STOP: final-v3 evidence already exists; preserve it: $FINAL_ROOT" >&2
  exit 93
fi
mkdir -p "$FINAL_ROOT"

PLAN="$ROOT/configs/eipm/n0/n0_v02_qsre_production_training_plan_v3.json"
RECOVERY_CONTRACT="$ROOT/configs/eipm/n0/n0_v02_qsre_final_production_recovery_contract_v1.json"
...
python - "$P1_RESULT" "$P1_ROOT" "$FAILED_P2_RESULT" "$FINAL_FREEZE" "$ORIGINAL_PRODUCTION_REVISION" "$P1_EXPECTED_SHA" "$PLAN" "$RECOVERY_CONTRACT" <<'PY'
import hashlib,json,sys
from pathlib import Path

p1_result=Path(sys.argv[1]); p1_root=Path(sys.argv[2]); p2_result=Path(sys.argv[3])
freeze_path=Path(sys.argv[4]); original_revision=sys.argv[5]; expected_p1_sha=sys.argv[6]
plan_path=Path(sys.argv[7]); recovery_path=Path(sys.argv[8])
...
assert plan["operator_architecture"]["open_schema_relation_descriptions_used_in_gradient"] is False
...
assert recovery["status"]=="FINAL_N0_PRODUCTION_ARCHITECTURE_PRECOMMITTED_BEFORE_NEXT_GPU_RUN"
assert recovery["compute"]["p2_v3_gpu_runs_authorized"]==1
assert recovery["compute"]["automatic_rerun"] is False
assert recovery["validation"]["reuse_original_frozen_final_validation"] is True

print("PASS_FINAL_N0_V3_SOURCE_LINEAGE")
print("p1_checkpoint_sha256="+digest)
print("failed_p2_status="+p2["status"])
print("frozen_final_validation_reused=true")
PY

echo "===== FINAL N0 PRODUCTION V3 ====="
date -Is
echo "source_revision=$HEAD"
echo "final_root=$FINAL_ROOT"
echo "p1_retrain=false"
echo "p2_v1_retrain=false"
echo "p2_v2_gpu_executed=false"
echo "p2_v3_gpu_runs_authorized=1"
echo "relation_hypotheses=continuous_until_structural_binding"
echo "exact_sparsity=structural_support_only"
echo "open_schema_descriptions_in_gradient=false"
...
echo "===== FINAL-P2: CONTINUOUS OPEN-SCHEMA OPERATOR V3 ====="
P2="$FINAL_ROOT/p2"
python "$ROOT/scripts/eipm/n0/train_n0_v02_qsre_production_p2_v3.py"   --plan "$PLAN"   --prepared-cache "$PROD_CACHE"   --schema-cache "$PROD_SCHEMA_CACHE"   --p1-result "$P1_RESULT"   --p1-root "$P1_ROOT"   --failed-p2-result "$FAILED_P2_RESULT"   --output-dir "$P2"
...
assert r["core_schema_only_query_training"] is True
assert r["continuous_relation_hypotheses"] is True
assert r["exact_relation_sparsity_before_binder"] is False
print("PASS_FINAL_OPERATOR_V3")
print("selected_step="+str(r["selected"]["step"]))
print("selected_checkpoint_sha256="+r["selected"]["checkpoint_sha256"])
PY

echo "===== FINAL-P3: EXACT-SPARSE STRUCTURAL BINDER ====="
P3="$FINAL_ROOT/p3"
python "$ROOT/scripts/eipm/n0/train_n0_v02_qsre_production_p3_v3.py"   --plan "$PLAN"   --prepared-cache "$PROD_CACHE"   --schema-cache "$PROD_SCHEMA_CACHE"   --p1-result "$P1_RESULT"   --p1-root "$P1_ROOT"   --p2-result "$P2/result.json"   --p2-root "$P2"   --output-dir "$P3"
...
assert r["open_schema_training_query_labels_used"] is False
assert r["open_schema_relation_descriptions_used_in_gradient"] is False
assert r["core_schema_only_binder_training"] is True
assert r["continuous_relation_hypotheses_from_operator"] is True
assert r["exact_sparsity_owned_by_binder"] is True
print("PASS_FINAL_BINDER_FRONTIER")
print("selected_step="+str(r["selected"]["step"]))
print("selected_checkpoint_sha256="+r["selected"]["checkpoint_sha256"])
PY

echo "===== FINAL-P4: FULLY PREDICTED QSRE; ZERO GRADIENT ====="
P4="$FINAL_ROOT/p4"
mkdir -p "$P4"
python "$ROOT/scripts/eipm/n0/eval_n0_v02_qsre_production_p4_v3.py"   --plan "$PLAN"   --prepared-cache "$PROD_CACHE"   --schema-cache "$PROD_SCHEMA_CACHE"   --p1-result "$P1_RESULT"   --p1-root "$P1_ROOT"   --p2-result "$P2/result.json"   --p2-root "$P2"   --p3-result "$P3/result.json"   --p3-root "$P3"   --output "$P4/result.json"

echo "===== FINAL NATIVE N0 OBJECTIVE VALIDATION ====="
FINAL_RESULT="$FINAL_ROOT/final-self-validation-result.json"
...
echo "===== FINAL N0 PRODUCTION V3 COMPLETE ====="
date -Is
