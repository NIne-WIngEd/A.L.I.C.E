...
if [[ "$HEAD" != "$EXPECTED" ]]; then
  echo "STOP: N0 closure-semantic source revision drift HEAD=$HEAD expected=$EXPECTED" >&2
  exit 90
fi
...
V1_CLOSURE="$WORKDIR/qsre-n0-closure-pass-v1"
V1_P2S_RESULT="$V1_CLOSURE/p2s-schema-matcher/result.json"
CLOSURE_ROOT="$WORKDIR/qsre-n0-closure-semantic-v2"

for root in "$SOURCE" "$RECOVERY" "$FAILED_FINAL" "$V1_CLOSURE"; do
  if [[ ! -d "$root" ]]; then
...
if [[ -e "$CLOSURE_ROOT" ]]; then
  echo "STOP: closure-semantic evidence already exists; preserve it: $CLOSURE_ROOT" >&2
  exit 93
fi
mkdir -p "$CLOSURE_ROOT"

PLAN="$ROOT/configs/eipm/n0/n0_v02_qsre_n0_closure_training_plan_v1.json"
MATCHER_PLAN="$ROOT/configs/eipm/n0/n0_v02_qsre_closure_matcher_plan_v2.json"
META_SCHEMA="$ROOT/configs/eipm/n0/n0_v02_qsre_closure_schema_meta_v2.json"
...
for required in \
  ...
  "$P1_RESULT" "$FAILED_P2_RESULT" "$V1_P2S_RESULT" \
...
do
  if [[ ! -e "$required" ]]; then
    echo "STOP: missing closure-semantic prerequisite: $required" >&2
    exit 94
  fi
done

python - "$P1_RESULT" "$P1_ROOT" "$FAILED_P2_RESULT" "$V1_P2S_RESULT" "$FINAL_FREEZE" \
...
v1_p2s_path=Path(sys.argv[4]); freeze_path=Path(sys.argv[5])
original_revision=sys.argv[6]; expected_p1_sha=sys.argv[7]
plan_path=Path(sys.argv[8]); matcher_plan_path=Path(sys.argv[9]); meta_path=Path(sys.argv[10])

p1=json.loads(p1_result.read_text())
p2=json.loads(failed.read_text())
v1_p2s=json.loads(v1_p2s_path.read_text())
...
assert v1_p2s["schema"]=="alice.eipm.n0.qsre-closure-matcher-result.v1"
assert v1_p2s["status"]=="FAIL_QSRE_CLOSURE_SCHEMA_MATCHER"
assert v1_p2s["selected"] is None
assert v1_p2s["p2_authorized"] is False
assert int(v1_p2s["history"][-1]["step"])==1000
assert int(v1_p2s["best_observed"]["step"])==850
best=v1_p2s["best_observed"]["metrics"]
expected={
    "auxiliary_holdout_relation_top1":0.5416666666666666,
    "auxiliary_seen_relation_top1":0.5625,
    "production_core_single_relation_top1":0.7090909090909091,
    "heldout_factor_macro_accuracy":0.6666666666666667,
}
for key,value in expected.items():
    assert math.isclose(float(best[key]),value,rel_tol=0.0,abs_tol=1e-12),(key,best[key])
assert v1_p2s["production_open_schema_descriptions_used_in_gradient"] is False
assert v1_p2s["auxiliary_holdout_relation_descriptions_used_in_gradient"] is False
...
assert matcher_plan["schema"]=="alice.eipm.n0.qsre-closure-matcher-plan.v2"
assert matcher_plan["source_failure"]["job"]==575962
assert matcher_plan["source_failure"]["status"]=="FAIL_QSRE_CLOSURE_SCHEMA_MATCHER"
assert matcher_plan["source_failure"]["p2_authorized"] is False
assert matcher_plan["architecture"]["fixed_semantic_anchor"] is True
assert matcher_plan["architecture"]["bidirectional_pair_refinement"] is True
assert matcher_plan["objective_revision"]["production_runtime_relation_text_grammar_reused_for_auxiliary_schema"] is True
assert matcher_plan["objective_revision"]["factor_train_phrase_bank_gradient_bearing"] is True
...
print("source_failed_p2s_job=575962")
print("source_failed_p2s_status="+v1_p2s["status"])
print("original_frozen_final_validation_reused=true")
PY

echo "===== N0 CLOSURE SEMANTIC V2 ====="
date -Is
echo "source_revision=$HEAD"
echo "closure_root=$CLOSURE_ROOT"
echo "source_failed_job=575958"
echo "source_failed_p2s_job=575962"
...
python "$ROOT/scripts/eipm/n0/train_n0_v02_qsre_closure_matcher_v2.py" \
  --plan "$MATCHER_PLAN" \
...
assert r["schema"]=="alice.eipm.n0.qsre-closure-matcher-result.v2"
...
assert r["matcher_report"]["factor_identity_parameters"]==0
assert r["matcher_report"]["fixed_semantic_anchor"] is True
assert r["matcher_report"]["bidirectional_pair_refinement"] is True
...
assert r["production_runtime_relation_text_grammar_reused"] is True
assert r["relation_keys_used_as_semantic_tokens"] is False
assert r["factor_train_phrases_used_in_gradient"] is True
assert r["auxiliary_pair_consistency_used_in_gradient"] is True
print("PASS_CLOSURE_SCHEMA_MATCHER_V2")
...
echo "===== N0 CLOSURE SEMANTIC V2 COMPLETE ====="
date -Is
