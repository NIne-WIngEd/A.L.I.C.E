...
RESULT_SCHEMA = "alice.eipm.n0.qsre-closure-matcher-result.v2"
CHECKPOINT_SCHEMA = "alice.eipm.n0.qsre-closure-matcher-checkpoint.v2"
...
def auxiliary_relation_schema_text(row: dict) -> str:
    """Use exactly the Production runtime relation-description grammar.

    Relation keys stay metadata only. The semantic input contains meaning,
    argument roles, and symmetry just as a dynamically supplied Production
    schema does.
    """
    return relation_schema_text(
        {
            "descriptions": [str(row["description"])],
            "source_argument": str(row["source_argument"]),
            "target_argument": str(row["target_argument"]),
            "symmetric": bool(row.get("symmetric", False)),
        }
    )


def factor_schema_text(category: str, description: str) -> str:
    # Factor keys are runtime indices/interfaces, not semantic label tokens.
    return f"Operator {category} meaning: {description}"
...
def factor_train_examples(meta: dict) -> dict[str, tuple[list[str], torch.Tensor]]:
    result: dict[str, tuple[list[str], torch.Tensor]] = {}
    for category in ("role", "traversal", "direction", "control"):
        texts: list[str] = []
        targets: list[int] = []
        for index, row in enumerate(meta["factors"][category]):
            for phrase in row["train_phrases"]:
                texts.append(
                    f"{phrase}. Which supplied {category} meaning best matches this instruction?"
                )
                targets.append(index)
    ...
    modifier_targets: list[tuple[int, int]] = []
    for modifier_index, row in enumerate(meta["factors"]["modifiers"]):
        for phrase in row["train_off"]:
            modifier_texts.append(
                f"{phrase}. Decide whether the {row['key'].lower()} modifier is active."
            )
            modifier_targets.append((modifier_index, 0))
        for phrase in row["train_on"]:
            modifier_texts.append(
                f"{phrase}. Decide whether the {row['key'].lower()} modifier is active."
            )
            modifier_targets.append((modifier_index, 1))
...
def encode_factor_queries(
    *,
    source: dict[str, tuple[list[str], torch.Tensor]],
    model,
    tokenizer,
    device: torch.device,
) -> dict:
...
def factor_instruction_losses(
    ...
    *,
    ...
    total = next(matcher.parameters()).sum() * 0.0
    parts: dict[str, float] = {}
    for category in ("role", "traversal", "direction", "control"):
        item = factor_queries[category]
        cache = factor_cache[category]
        output = matcher(
            query_hidden_states=item["states"].to(device, dtype=torch.float32),
            query_token_mask=item["mask"].to(device).bool(),
            schema_hidden_states=cache["token_states"].to(device, dtype=torch.float32),
            schema_token_mask=cache["token_mask"].to(device).bool(),
        )
        loss = F.cross_entropy(
            output["logits"],
            item["targets"].to(device).long(),
        )
        total = total + float(weights[category]) * loss
        parts[category] = float(loss.detach().item())
...
def main() -> None:
    ...
    if plan.get("schema") != "alice.eipm.n0.qsre-closure-matcher-plan.v2":
    ...
    if meta.get("schema_text_contract", {}).get("production_runtime_encoding_aligned") is not True:
        raise SystemExit("closure schema text is not Production-runtime aligned")
    if meta.get("schema_text_contract", {}).get("relation_key_in_semantic_text") is not False:
        raise SystemExit("relation key entered semantic schema text")
    ...
    factor_train_queries = encode_factor_queries(
        source=factor_train_examples(meta),
        model=semantic_model,
        tokenizer=tokenizer,
        device=device,
    )
    factor_holdout = encode_factor_queries(
    ...
    for step in range(1, max_steps + 1):
        ...
        factor_production_total, factor_production_parts = factor_losses(
            matcher,
            query=factor_query,
        ...
        factor_phrase_total, factor_phrase_parts = factor_instruction_losses(
            matcher,
            factor_queries=factor_train_queries,
            factor_cache=factor_cache,
            weights=factor_weights,
            device=device,
        )
        # Preserve the precommitted aggregate factor weight while aligning the
        # train domain with the held-out factor-instruction evaluation domain.
        factor_total = 0.5 * (
            factor_production_total + factor_phrase_total
        )
        factor_parts = {
            key: 0.5 * (
                factor_production_parts[key] + factor_phrase_parts[key]
            )
            for key in factor_production_parts
        }

        other_view = 1 - view
        core_pair_output = matcher(
        ...
        core_pair_loss = F.kl_div(
            F.log_softmax(core_logits, dim=-1),
            F.softmax(core_pair_output["logits"], dim=-1),
            reduction="batchmean",
        )
        aux_other_bank = train_q1 if view == 0 else train_q0
        aux_other_mask = train_qmask1 if view == 0 else train_qmask
        aux_pair_output = matcher(
            query_hidden_states=aux_other_bank[aux_idx].to(
                device, dtype=torch.float32
            ),
            query_token_mask=aux_other_mask[aux_idx].to(device).bool(),
            schema_hidden_states=train_aux_states.to(device, dtype=torch.float32),
            schema_token_mask=train_aux_mask.to(device).bool(),
        )
        aux_pair_loss = F.kl_div(
            F.log_softmax(aux_logits, dim=-1),
            F.softmax(aux_pair_output["logits"], dim=-1),
            reduction="batchmean",
        )
        pair_loss = 0.5 * (core_pair_loss + aux_pair_loss)
    ...
    result = {
        ...
        "production_runtime_relation_text_grammar_reused": True,
        "relation_keys_used_as_semantic_tokens": False,
        "factor_train_phrases_used_in_gradient": True,
        "auxiliary_pair_consistency_used_in_gradient": True,
