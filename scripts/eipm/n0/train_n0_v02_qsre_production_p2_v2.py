...
def operator_supervised_loss_v2(
    *,
    model_output: dict,
    split: dict,
...
def infer_view_output(
    *,
    ...
    operator_model: QSREProductionOperatorInducerV2,
    max_steps: int,
    device: torch.device,
):
...
def save_checkpoint(
    ...
    torch.save(
        {
            "schema": "alice.eipm.n0.qsre-production-p2-checkpoint.v2",
            "architecture": "open-schema-factorized-operator-v2",
            "stage": "P2",
            "step": step,
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise SystemExit("P2 v2 requires CUDA")
    output_dir = Path(args.output_dir)
    if output_dir.exists():
        raise SystemExit("refusing to overwrite P2 v2 evidence")
    ...
    if failed_p2.get("status") != "FAIL_QSRE_PRODUCTION_P2_OPERATOR":
        raise SystemExit("P2 v2 requires preserved genuine P2 failure evidence")
    ...
    if (
        ...
        raise SystemExit("P2 v2 boundary violation")
    ...
    operator_model = QSREProductionOperatorInducerV2(config).to(device)

    # One shared semantic metric is initialized in the proven P1 schema space
    # and is applied symmetrically to both query and runtime schema text.
    with torch.no_grad():
    ...
    # Training uses the full runtime schema as candidate context. The four
    # zero-shot relations never receive training query labels, but they are
    # present as runtime hard negatives. This prevents a 6->10 candidate-set
    # shift from becoming an untested serving condition.
    with torch.no_grad():
        encoded_runtime = schema_encoder(full_schema)
    ...
    while step < max_train_steps:
        ...
        oracle = oracle_operator_from_targets(
            train,
            idx,
            schema_relation_state=encoded_runtime["schema_relation_state"],
            config=config,
            device=device,
        )
        view_ops = []
        ...
        for view in (0, 1):
            ...
            down = execute_with_oracle_support(
                ...
                indices=idx,
                operator=op,
                schema_relation_state=encoded_runtime[
        ...
        identity_logits = operator_model.schema_identity_logits(full_schema)
        ...
        if not torch.isfinite(loss):
            raise RuntimeError("P2 v2 nonfinite loss")
        loss.backward()
        ...
        if step % eval_every == 0 or step == max_train_steps:
            ...
            print(
                "P2_V2_EVAL=" + json.dumps(record, sort_keys=True),
                flush=True,
            )
    ...
    status = (
        ...
        if selected
        else "FAIL_QSRE_PRODUCTION_P2_OPERATOR_V2"
    )
    result = {
        "schema": "alice.eipm.n0.qsre-production-p2-result.v2",
        "architecture": "open-schema-factorized-operator-v2",
        "status": status,
        "selected": selected,
        "best_observed": best,
        ...
        "open_schema_runtime_candidates_used": True,
    ...
    print(
        "P2_V2_RESULT=" + json.dumps(result, sort_keys=True),
        flush=True,
    )
    if not selected:
        raise SystemExit(31)
