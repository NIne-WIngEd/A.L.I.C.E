...
def save_checkpoint(path:Path,binder,config,step,lineage)->str:
    ...
    torch.save({
        "schema":"alice.eipm.n0.qsre-production-p3-checkpoint.v1",
...
def main()->None:
    ...
    while step<max_train_steps:
        ...
        for view in (0,1):
            bound=bind(
                split=train,indices=idx,view=view,schema=train_schema,
            ...
            if bool(valid.any()):
                support_loss=F.binary_cross_entropy_with_logits(
                    bound["support_logits"][valid],target[valid],
                )
            else:
    ...
    result={
        "schema":"alice.eipm.n0.qsre-production-p3-result.v1",
