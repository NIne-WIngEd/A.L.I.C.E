...
def save_checkpoint(path:Path,binder,config,step,lineage)->str:
    ...
    torch.save({
        "schema":"alice.eipm.n0.qsre-production-p3-checkpoint.v2",
        "architecture":"open-schema-binder-v2",
...
def main()->None:
    ...
    with torch.no_grad():
        encoded_train=schema_encoder(full_schema)
        encoded_dev=encoded_train
    ...
    while step<max_train_steps:
        ...
        for view in (0,1):
            bound=bind(
                split=train,indices=idx,view=view,schema=full_schema,
    ...
    result={
        "schema":"alice.eipm.n0.qsre-production-p3-result.v2",
        "architecture":"open-schema-binder-v2",
        ...
        "full_runtime_schema_candidates_used":True,
