...
QUAL_SCHEMA = "alice.eipm.n0.query-edge-competitive-router-qualification.v0.1"
...
CHECKPOINT_SCHEMA = "alice.eipm.n0.competitive-edge-router-training-checkpoint.v0.1"
RESULT_SCHEMA = "alice.eipm.n0.competitive-edge-router-training-result.v0.1"
...
def candidate_forward(
    graph: CompetitiveQueryEdgeEvidenceGraphEncoder,
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise SystemExit("competitive edge-router training requires one CUDA device")
    ...
    for path in (
        ...
        if not path.is_file():
            raise SystemExit(f"missing competitive training input: {path}")
    if output.exists() and any(output.iterdir()):
        raise SystemExit(f"refusing to overwrite competitive training output: {output}")
    ...
    if qual.get("schema") != QUAL_SCHEMA:
        raise SystemExit("competitive qualification schema drift")
    ...
    for key in (
        ...
        if qual.get(key) is not True:
            raise SystemExit(f"competitive qualification invariant failed: {key}")
    ...
    cache_path = output / "competitive_query_edge_train_dev_hidden_cache.pt"
    ...
    if any(
        ...
        raise SystemExit(
            f"competitive parent load drift missing={list(missing)} unexpected={list(unexpected)}"
        )

    trainable = graph.freeze_parent_for_bridge_training()
    parent_hash_before = old.tensor_state_sha256(
        graph, exclude_prefixes=("competitive_query_edge_bridge.",)
    )
    ...
    if not initial_preserved:
        raise SystemExit("zero-init competitive bridge failed preservation policy")
    ...
    if not torch.equal(parent_out["field_weights"], candidate_out["field_weights"]):
        raise SystemExit("competitive zero-init field weights lost exact parent")
    if not torch.equal(parent_out["pooled_state"], candidate_out["pooled_state"]):
        raise SystemExit("competitive zero-init pooled state lost exact parent")

    print(
        "baseline_competitive_dev=" + json.dumps(baseline_dev, sort_keys=True),
        flush=True,
    )
    print(
    ...
    for step in range(1, args.max_steps + 1):
        ...
        if step % args.save_every == 0 or step == args.max_steps:
            ...
            candidate_path = cp / "competitive_query_edge_bridge.safetensors"
            bridge_state = {
                name: value.detach().cpu().contiguous()
                for name, value in graph.state_dict().items()
                if name.startswith("competitive_query_edge_bridge.")
            }
            ...
            receipt = {
                ...
                "trainable_scope": "competitive_query_edge_bridge_only",
                ...
                "parent_parameters_exactly_unchanged": (
                    old.tensor_state_sha256(
                        graph,
                        exclude_prefixes=("competitive_query_edge_bridge.",),
                    )
                    == parent_hash_before
                ),
    ...
    result = {
        ...
        "status": (
            "PASS_COMPETITIVE_EDGE_ROUTER_CAUSAL_DEV_AND_PRESERVATION_READY_FOR_SEPARATE_HELDOUT_DECISION"
        ...
        "baseline_competitive_dev": baseline_dev,
        ...
        "next_action": (
            ...
            "stop_and_reassess_competitive_route_or_training_boundary_from_first_principles"
        ),
    }
