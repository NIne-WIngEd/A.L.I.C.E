...
PREP_SCHEMA = "alice.eipm.n0.v02-relation-semantic-grounding-preparation.v0.1"
MANIFEST_SCHEMA = "alice.eipm.n0.v02-relation-semantic-grounding-curriculum.v0.1"
CHECKPOINT_SCHEMA = "alice.eipm.n0.v02-relation-semantic-grounding-checkpoint.v0.1"
RESULT_SCHEMA = "alice.eipm.n0.v02-relation-semantic-grounding-result.v0.1"
...
def semantic_role_summary(metrics: dict[str, Any]) -> dict[str, float]:
...
def endpoint_preservation_ok(
    ...
    *,
    ...
    return (
        float(current["pair_accuracy"]) >= float(baseline["pair_accuracy"]) - pair_tolerance
        and float(current["family_min_pair_accuracy"]) >= float(baseline["family_min_pair_accuracy"]) - family_tolerance
        and float(current["row_accuracy"]) >= float(baseline["row_accuracy"]) - row_tolerance
    )
...
def ordinary_replay_ok(
    ...
    *,
    ...
    return (
        float(current["family_macro_target_support_mass"]) >= float(baseline["family_macro_target_support_mass"]) - macro_tolerance
        and float(current["family_min_target_support_mass"]) >= float(baseline["family_min_target_support_mass"]) - min_tolerance
        and float(current["family_macro_top1_support_accuracy"]) >= float(baseline["family_macro_top1_support_accuracy"]) - top1_tolerance
    )
...
def save_checkpoint(
    *,
    ...
    save_file(
        {k: v.detach().cpu().contiguous() for k, v in graph.state_dict().items()},
        str(weights),
    )
    ...
    receipt = {
        ...
        "semantic_grounding_dev_role_summary": semantic_role_summary(semantic_dev),
...
def main() -> None:
    ...
    p.add_argument("--learning-rate", type=float, default=7.5e-5)
    p.add_argument("--weight-decay", type=float, default=0.03)
    p.add_argument("--warmup-steps", type=int, default=12)
    p.add_argument("--semantic-ce-weight", type=float, default=0.55)
    p.add_argument("--pair-flip-weight", type=float, default=0.15)
    p.add_argument("--ordinary-replay-weight", type=float, default=0.15)
    p.add_argument("--endpoint-replay-weight", type=float, default=0.15)
    ...
    p.add_argument("--test-family-min-pair-accuracy", type=float, default=0.75)
    p.add_argument("--test-row-accuracy", type=float, default=0.95)
    p.add_argument("--seed", type=int, default=20260918)
    args = p.parse_args()

    if not torch.cuda.is_available():
        raise SystemExit("relation-semantic grounding repair requires one CUDA device")
    ...
    if prep.get("schema") != PREP_SCHEMA or prep.get("status") != "PASS_RELATION_SEMANTIC_GROUNDING_READY_FOR_GPU":
        raise SystemExit("relation-semantic preparation contract mismatch")
    if prep.get("git_revision") != git_revision(root):
        raise SystemExit("relation-semantic preparation git drift")
    if prep.get("hard_parameter_ceiling") is not None or prep.get("hard_parameter_floor") is not None:
        raise SystemExit("relation-semantic preparation capacity contract drift")
    if prep.get("scale_authorized") is not False:
        raise SystemExit("relation-semantic preparation scaling contract drift")

    curriculum_path = Path(args.curriculum).resolve()
    manifest_path = Path(args.manifest).resolve()
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if manifest.get("schema") != MANIFEST_SCHEMA:
        raise SystemExit("relation-semantic manifest schema mismatch")
    if manifest.get("compiled_sha256") != sha(curriculum_path):
        raise SystemExit("relation-semantic curriculum hash mismatch")
    for key in (
    ...
    if manifest.get("query_names_endpoint_role_explicitly") is not False:
        raise SystemExit("semantic curriculum leaked explicit endpoint-role wording")
    if manifest.get("graph_pooled_cosine_is_optimization_target") is not False:
        raise SystemExit("relation-semantic objective drift")
    ...
    for key, path in check_paths.items():
        if not path.is_file():
            raise SystemExit(f"missing relation-semantic artifact: {key}={path}")
        if hash_inputs.get(key) != sha(path):
            raise SystemExit(f"relation-semantic artifact drift: {key}")

    output = Path(args.output_dir).resolve()
    if output.exists() and any(output.iterdir()):
        raise SystemExit(f"refusing to overwrite relation-semantic output: {output}")
    ...
    semantic_state = load_file(
        str(Path(args.semantic_checkpoint).resolve() / "alice_n0_v02.safetensors"),
        device="cpu",
    )
    ...
    structured.load_state_dict(
        load_file(
            str(Path(args.structured_checkpoint).resolve() / "structured_state.safetensors"),
            device="cpu",
        ),
        strict=True,
    )
    ...
    print("relation_semantic_cache_start=true", flush=True)
    ...
    cache_path = output / "relation_semantic_grounding_cache.pt"
    torch.save(payload, cache_path)
    print(f"relation_semantic_cache_complete=true path={cache_path}", flush=True)
    del semantic, semantic_state, structured
    torch.cuda.empty_cache()

    ordinary_payload = torch.load(Path(args.ordinary_replay_cache).resolve(), map_location="cpu")
    endpoint_payload = torch.load(Path(args.endpoint_replay_cache).resolve(), map_location="cpu")
    ...
    trainable = endpoint.set_endpoint_read_trainable(graph)
    if not trainable:
        raise SystemExit("relation-semantic repair has no trainable relation-read parameters")

    semantic_train_pairs = rr.PairDataset(payload, rr.pair_indices(payload, "train"))
    semantic_dev_pairs = rr.PairDataset(payload, rr.pair_indices(payload, "dev"))
    semantic_test_pairs = rr.PairDataset(payload, rr.pair_indices(payload, "test"))

    ordinary_train_indices = [i for i, split in enumerate(ordinary_payload["splits"]) if split == "train"]
    ordinary_dev_indices = [i for i, split in enumerate(ordinary_payload["splits"]) if split == "dev"]
    ...
    endpoint_train_indices = [i for i, split in enumerate(endpoint_payload["splits"]) if split == "train"]
    endpoint_train = rr.RowDataset(endpoint_payload, endpoint_train_indices)
    endpoint_dev_pairs = rr.PairDataset(endpoint_payload, rr.pair_indices(endpoint_payload, "dev"))

    initial_semantic_dev = sr.evaluate_pairs(
        graph, adapter, semantic_dev_pairs, payload, device, args.eval_batch_size
    )
    baseline_ordinary = sr.evaluate_replay(
        baseline_graph, adapter, ordinary_dev, ordinary_payload, device, args.eval_batch_size
    )
    baseline_endpoint_dev = sr.evaluate_pairs(
        baseline_graph, adapter, endpoint_dev_pairs, endpoint_payload, device, args.eval_batch_size
    )
    print("initial_semantic_dev=" + json.dumps(initial_semantic_dev, sort_keys=True), flush=True)
    print("initial_semantic_role_summary=" + json.dumps(semantic_role_summary(initial_semantic_dev), sort_keys=True), flush=True)
    print("baseline_ordinary_replay=" + json.dumps(baseline_ordinary, sort_keys=True), flush=True)
    print("baseline_endpoint_dev=" + json.dumps(baseline_endpoint_dev, sort_keys=True), flush=True)
    ...
    for step in range(1, args.max_steps + 1):
        ...
        semantic_ce = 0.5 * (
            endpoint.distribution_ce(a_out["field_weights"], a["target_distribution"])
            + endpoint.distribution_ce(b_out["field_weights"], b["target_distribution"])
        )
        pair_flip = rr.pair_flip_loss(a_out, b_out, a, b, margin=0.25)

        current_ordinary = rr.graph_forward(graph, adapter, ordinary_batch)
        current_endpoint = rr.graph_forward(graph, adapter, endpoint_batch)
        with torch.inference_mode():
            baseline_ordinary_batch = rr.graph_forward(baseline_graph, adapter, ordinary_batch)
            baseline_endpoint_batch = rr.graph_forward(baseline_graph, adapter, endpoint_batch)
        ...
        if step == 1 or step % 20 == 0:
            print(json.dumps({
                ...
                "ordinary_replay_kl": float(ordinary_kl.detach().cpu()),
                "endpoint_replay_kl": float(endpoint_kl.detach().cpu()),
        ...
        if step % args.save_every == 0 or step == args.max_steps:
            ...
            endpoint_dev = sr.evaluate_pairs(
                graph, adapter, endpoint_dev_pairs, endpoint_payload, device, args.eval_batch_size
            )
            ...
            print("relation_semantic_checkpoint=" + json.dumps(receipt, sort_keys=True), flush=True)
    ...
    def score(receipt: dict[str, Any]) -> tuple[float, ...]:
        ...
        return (
            1.0 if preservation_ok(receipt) else 0.0,
            float(dev["family_min_pair_accuracy"]),
            min(float(roles["source_semantic_pair_accuracy"]), float(roles["target_semantic_pair_accuracy"])),
    ...
    graph.load_state_dict(
        load_file(str(output / winner_key / "evidence_graph_dual_endpoint.safetensors"), device="cpu"),
        strict=True,
    )
    graph.to(device).eval()

    semantic_test = sr.evaluate_pairs(
        graph, adapter, semantic_test_pairs, payload, device, args.eval_batch_size
    )
    ...
    passed = (
        ...
        and float(semantic_test["family_min_pair_accuracy"]) >= args.test_family_min_pair_accuracy
    ...
    result = {
        ...
        "status": (
            "PASS_RELATION_SEMANTIC_GROUNDING_HELDOUT_READY_FOR_ONE_DOWNSTREAM_CAUSAL_CHECK"
            if passed else
        ...
        "initial_semantic_role_summary": semantic_role_summary(initial_semantic_dev),
        ...
        "selection_policy": (
            "preserve_ordinary_and_endpoint_parent_then_worst_family_semantic_dev_pair_accuracy_"
            "then_balanced_source_target_semantics_then_dev_pair_accuracy_then_row_accuracy_"
            "then_graph_margin_then_earlier_checkpoint"
        ),
        "semantic_heldout_test_evaluated_once": True,
        "semantic_heldout_test_metrics": semantic_test,
        "semantic_heldout_role_summary": semantic_role_summary(semantic_test),
        ...
        "heldout_gate": {
            ...
            "ordinary_replay_macro_tolerance": args.ordinary_replay_macro_tolerance,
            "ordinary_replay_min_tolerance": args.ordinary_replay_min_tolerance,
            "ordinary_replay_top1_tolerance": args.ordinary_replay_top1_tolerance,
            "endpoint_dev_pair_tolerance": args.endpoint_dev_pair_tolerance,
            "endpoint_dev_family_tolerance": args.endpoint_dev_family_tolerance,
            "endpoint_dev_row_tolerance": args.endpoint_dev_row_tolerance,
        ...
        "semantic_parent_mutated": False,
        "structured_parent_mutated": False,
        "relation_endpoint_repair_step200_used_as_parent": True,
        ...
        "n0_complete": False,
        "next_action": (
            "run_one_fresh_downstream_causal_check_on_independent_relation_semantic_analogues_without_rerunning_frozen_challenge"
            if passed else
            "inspect_single_heldout_relation_semantic_failure_and_choose_one_causal_next_change"
        ),
    }
