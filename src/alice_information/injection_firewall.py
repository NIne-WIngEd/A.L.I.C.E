...
_RULES = (
    _PatternRule(
        ...
        re.compile(
            r"(?:BEGIN|END)\s+(?:UNTRUSTED\s+EXTERNAL\s+SOURCE|SOURCE\s+CONTENT|VERIFIED\s+SOURCE\s+INSPECTION|VERIFIED\s+SOURCE\s+FRESHNESS|VERIFIED\s+WEB\s+GROUNDING)|ALICE-(?:EXTERNAL-SOURCE|INJECTION-INSPECTION|FRESHNESS-ASSESSMENT|WEB-GROUNDING)-|\[WEB:[^\]\r\n]{1,100}\]",
            _FLAGS,
        ),
    ),
    _PatternRule(
