...
from .query_edge_cross_attention_bridge import QueryEdgeCrossAttentionBridge


class QueryEdgeBridgeEvidenceGraphEncoder(DualEndpointEvidenceGraphEncoder):
    """Frozen proven parent graph plus a zero-gated edge-specific specialist.

    The parent remains the authoritative shared graph expert.  The specialist
    sees the actual source and target graph states before reading query tokens,
    so it can bind language intent to one concrete edge rather than learning a
    relation-global SOURCE/TARGET polarity.

    At initialization the specialist gate is exactly zero.  Therefore, after a
    parent checkpoint is loaded, field weights and pooled state are exact parent
    outputs.  The specialist proposes independent source and target residuals;
    they are not forced to sum to zero.
    """
    ...
    def __init__(
        ...
        *,
        ...
        bridge: QueryEdgeCrossAttentionBridge,
    ) -> None:
    ...
    def _bridge_relation_bias(
        ...
        *,
        ...
        active = bridge["relation_has_interface"].to(x.dtype)
    ...
    def freeze_parent_for_bridge_training(self) -> list[nn.Parameter]:
        bridge_names = set(self.bridge_parameter_names())
        for name, parameter in self.named_parameters():
            parameter.requires_grad = name in bridge_names
        trainable = [
            parameter
            for name, parameter in self.named_parameters()
        ...
        if not trainable:
            raise RuntimeError("query-edge bridge has no trainable parameters")
        return trainable
    ...
    def parameter_report(self) -> dict[str, Any]:
        ...
        report["query_edge_bridge_parameters"] = sum(
            p.numel()
            for name, p in self.named_parameters()
            if name in names
        )
        report["specialist_input"] = (
            "relation_conditioned_intermediate_query_tokens_plus_actual_"
            "source_and_target_graph_states"
        )
        report["specialist_composition"] = (
            "independent_source_target_residuals_through_zero_initialized_"
            "edge_specific_gate_on_frozen_parent_pool_logits"
        )
        report["edge_specific_query_binding"] = True
        report["independent_source_target_residuals"] = True
        report["forced_antisymmetry"] = False
        report["generic_endpoint_classifier"] = False
