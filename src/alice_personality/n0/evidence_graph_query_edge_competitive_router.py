...
from .query_edge_competitive_router_bridge import CompetitiveQueryEdgeBridge


class CompetitiveQueryEdgeEvidenceGraphEncoder(DualEndpointEvidenceGraphEncoder):
    """Frozen dual-endpoint parent plus competitive query-edge specialist.

    The bridge's route distribution is the runtime contribution controller.
    Route class zero is the parent/no-op choice.  Edge classes compete across
    all active directed edges.  Source/target residual proposals are independent
    and are scaled by the selected edge probabilities.
    """
    ...
    def __init__(
        ...
        *,
        ...
        bridge: CompetitiveQueryEdgeBridge,
    ) -> None:
        ...
        self.competitive_query_edge_bridge = bridge
    ...
    def _bridge_relation_bias(
        ...
        *,
        ...
        bridge = self.competitive_query_edge_bridge(
    ...
    def freeze_parent_for_bridge_training(self) -> list[nn.Parameter]:
        ...
        if not trainable:
            raise RuntimeError("competitive query-edge bridge has no trainable parameters")
        return trainable
    ...
    def parameter_report(self) -> dict[str, Any]:
        ...
        report["competitive_query_edge_bridge_parameters"] = sum(
            p.numel()
            for name, p in self.named_parameters()
            if name in names
        )
        report["edge_specific_query_binding"] = True
        report["competitive_edge_routing"] = True
