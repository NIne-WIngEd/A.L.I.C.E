...
from .query_edge_dual_view_late_interaction import (
    DualViewLateInteractionBridge,
)


class DualViewLateInteractionEvidenceGraphEncoder(
    DualEndpointEvidenceGraphEncoder
):
    """Frozen graph parent plus dual-view late-interaction specialist."""

    def __init__(
        self,
        *,
        config: EvidenceGraphConfig,
        bridge: DualViewLateInteractionBridge,
    ) -> None:
        ...
        self.dual_view_query_edge_bridge = bridge
    ...
    def _bridge_relation_bias(
        ...
        *,
        ...
        query_content_mask: torch.Tensor | None,
        field_token_states: torch.Tensor | None,
        field_content_mask: torch.Tensor | None,
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor] | None]:
        batch, fields, _ = x.shape
        zero = torch.zeros(batch, fields, device=x.device, dtype=x.dtype)
        supplied = (
            query_hidden_states is not None,
            query_attention_mask is not None,
            query_content_mask is not None,
            field_token_states is not None,
            field_content_mask is not None,
        )
        if not any(supplied):
            return zero, None
        if not all(supplied):
            raise ValueError(
                "dual-view bridge inputs must be supplied together"
            )

        bridge = self.dual_view_query_edge_bridge(
    ...
    def forward(
        ...
        *,
        ...
        query_content_mask: torch.Tensor | None = None,
        field_token_states: torch.Tensor | None = None,
        field_content_mask: torch.Tensor | None = None,
        ...
        if base_field_weights is not None and self.config.base_weight_scale > 0.0:
            ...
            pool_scores = (
                pool_scores
                + self.config.base_weight_scale * base_prior
            )
    ...
    def freeze_parent_for_bridge_training(self) -> list[nn.Parameter]:
        ...
        if not trainable:
            raise RuntimeError(
                "dual-view late-interaction bridge has no trainable parameters"
            )
        return trainable
    ...
    def parameter_report(self) -> dict[str, Any]:
        ...
        report["dual_view_query_edge_bridge_parameters"] = sum(
            p.numel()
            for name, p in self.named_parameters()
            if name in names
        )
        report["content_binding_channel"] = (
            "final_semantic_query_tokens_to_final_semantic_field_tokens"
        )
        report["reasoning_channel"] = (
            "frozen_parent_structured_graph_plus_relation_multilayer_query"
        )
        report["hierarchical_parent_specialist_routing"] = True
        report["edge_binding_bypasses_structured_compression"] = True
        report["edge_binding_uses_graph_state"] = False
        report["parent_graph_can_remain_frozen"] = True
        report["setwise_router_transformer"] = False
