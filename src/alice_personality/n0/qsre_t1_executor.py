...
class QSRET1Executor(nn.Module):
    ...
    This module intentionally has no natural-language operator extractor,
    no learned support selector, and no parent-global-logit input.
    """
    ...
    def forward(
        ...
        *,
        ...
        if bool(edge_valid_mask.any()):
            ...
            if int(valid_rel.min()) < 0 or int(valid_rel.max()) >= self.config.num_relations:
        ...
        control_state = qsre_control_state(operator.applicability, has_support)

        node = (
            self.node_projection(field_state)
            + self.field_metadata_projection(field_metadata)
            + operator.focus_field_weight.unsqueeze(-1) * self.focus_embedding.view(1, 1, -1)
        )
        ...
        edge_rel_state = self.relation_embedding(edge_relation_id.clamp(
            min=0,
            max=self.config.num_relations - 1,
        ))
        edge_meta_state = self.edge_metadata_projection(edge_metadata)
        source_position = self.endpoint_position_embedding.weight[0].view(1, 1, -1)
        target_position = self.endpoint_position_embedding.weight[1].view(1, 1, -1)

        batch_index = torch.arange(batch, device=field_state.device).unsqueeze(1)
        ...
        for step in range(steps):
            ...
            relation_match = edge_relation_id.eq(requested_relation[:, None])
            gate = (
                edge_support_weight
                * edge_valid_mask.to(edge_support_weight.dtype)
                * relation_match.to(edge_support_weight.dtype)
                * step_active[:, None].to(edge_support_weight.dtype)
            )

            msg_input = torch.cat([edge_state, q_edge], dim=-1)
            source_message = self.source_message(msg_input) * gate.unsqueeze(-1)
            target_message = self.target_message(msg_input) * gate.unsqueeze(-1)
        ...
        relation_mask_f = operator.relation_sequence_mask.to(field_state.dtype)
        ...
        relation_count = relation_mask_f.sum(dim=1, keepdim=True).clamp_min(1.0)
        operator_summary = base_operator + relation_sum / relation_count

        q_readout = operator_summary[:, None, :].expand(batch, fields, -1)
        readout_input = torch.cat([node, q_readout], dim=-1)
        ...
        relational_probability = (
            relational_probability
            * relational_rows[:, None].to(relational_probability.dtype)
        )
