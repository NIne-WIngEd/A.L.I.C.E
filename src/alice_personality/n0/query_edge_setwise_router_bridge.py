...
class SetwiseQueryEdgeBridge(nn.Module):
    """Query-edge bridge with joint, permutation-equivariant edge arbitration.

    The previously qualified query-edge representation path is retained.  The
    routing control plane is changed in one place: active directed edges are
    contextualized together before route logits are produced.  Softmax-only
    competition between independently scored edges is therefore removed.

    Route classes:
      0        -> frozen parent / no-op
      1..E     -> active directed graph edges

    Exact-parent initialization is guaranteed by zero-initialized source and
    target residual readouts.  Route logits may be non-uniform at
    initialization because they have no effect until a residual proposal is
    non-zero.
    """
    ...
    def __init__(
        ...
        *,
        ...
        route_context_layers: int = 2,
        route_context_heads: int = 8,
        route_context_ffn_multiplier: int = 4,
    ) -> None:
        super().__init__()
        ...
        if num_hidden_states < 2:
            raise ValueError("setwise bridge requires hidden-state depth")
        ...
        if route_context_layers < 1:
            raise ValueError("route_context_layers must be positive")
        if route_context_heads < 1 or width % route_context_heads != 0:
            raise ValueError(
                "route_context_heads must be positive and divide interface_size"
            )
        if route_context_ffn_multiplier < 1:
            raise ValueError("route_context_ffn_multiplier must be positive")
        ...
        self.route_context_layers = int(route_context_layers)
        self.route_context_heads = int(route_context_heads)
        ...
        # The no-op route must see the query too.  A global query summary is
        # added to every route node and to the learned no-op token.
        self.global_query_projection = nn.Linear(
            semantic_size,
            width,
            bias=False,
        )
        self.noop_route_token = nn.Parameter(torch.zeros(width))

        route_layer = nn.TransformerEncoderLayer(
            d_model=width,
            nhead=route_context_heads,
            dim_feedforward=route_context_ffn_multiplier * width,
            dropout=dropout,
            activation="gelu",
            batch_first=True,
            norm_first=True,
        ...
        self.route_set_encoder = nn.TransformerEncoder(
            route_layer,
            num_layers=route_context_layers,
        )
        ...
        # Exact-parent initialization depends only on these readouts being zero.
        # The contextual router itself is intentionally live at initialization
        # so route supervision can update the whole set encoder immediately.
    ...
    def from_layer_map_file(
        ...
        *,
        ...
        return cls(
            ...
            route_context_layers=route_context_layers,
            route_context_heads=route_context_heads,
            route_context_ffn_multiplier=route_context_ffn_multiplier,
        )
    ...
    def _masked_query_summary(
        ...
        mask = attention_mask.to(final_hidden.dtype).unsqueeze(-1)
        denom = mask.sum(dim=1).clamp_min(1.0)
        return (final_hidden * mask).sum(dim=1) / denom
    ...
    def forward(
        ...
        *,
        ...
        global_query = self.global_query_projection(
            self._masked_query_summary(final_hidden, attention_mask)
        )
        edge_route_nodes = fused + global_query[:, None, :]
        noop_route_node = self.noop_route_token[None, :] + global_query
        route_nodes = torch.cat(
            [noop_route_node[:, None, :], edge_route_nodes],
            dim=1,
        )

        route_padding_mask = torch.cat(
            [
                torch.zeros(
                    batch,
                    1,
                    dtype=torch.bool,
                    device=route_active.device,
                ),
                ~route_active,
            ],
            dim=1,
        )
        contextual_route_nodes = self.route_set_encoder(
            route_nodes,
            src_key_padding_mask=route_padding_mask,
        )
        route_logits_with_noop = self.route_score(
            contextual_route_nodes
        ).squeeze(-1)
        edge_route_logits = route_logits_with_noop[:, 1:].masked_fill(
            ~route_active,
            -1.0e4,
        )
        route_logits_with_noop = torch.cat(
            [route_logits_with_noop[:, :1], edge_route_logits],
            dim=-1,
        )
        ...
        return {
            ...
            "route_contextual_edge_state": contextual_route_nodes[:, 1:],
            "route_contextual_noop_state": contextual_route_nodes[:, 0],
    ...
    def parameter_report(self) -> dict[str, Any]:
        return {
            ...
            "route_context_layers": self.route_context_layers,
            "route_context_heads": self.route_context_heads,
            ...
            "zero_initialized_route_head": False,
