...
def encoded_schema_inputs(
    ...
    # Operator-v2 deliberately does not use encoded schema-token states as
...
def run(
    model: QSREProductionOperatorInducerV2,
    *,
    q: torch.Tensor,
    qmask: torch.Tensor,
    s: QSREDynamicRelationSchema,
...
def test_v2_schema_permutation_equivariance() -> None:
...
def test_v2_relation_scale_cannot_suppress_stop_unknown_event() -> None:
...
def test_v2_unknown_and_stop_are_distinct_events() -> None:
...
def test_v2_schema_identity_logits_are_permutation_equivariant() -> None:
...
def test_v2_parameter_report_has_no_schema_or_hop_parameter_axis() -> None:
    report = QSREProductionOperatorInducerV2(cfg()).parameter_report()
