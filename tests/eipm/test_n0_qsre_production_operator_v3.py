...
def test_v3_query_coverage_is_monotone_and_bounded() -> None:
    torch.manual_seed(60)
    model = QSREProductionOperatorInducerV3(cfg()).eval()
    q, qmask = query(batch=2, tokens=9, seed=61)
    s = schema(5, seed=62)
    out = run(model, q=q, qmask=qmask, s=s, max_steps=3)
    coverage = out["step_query_coverage"]
    remaining = out["step_query_remaining"]

    assert coverage.shape == (2, 3, 9)
    assert remaining.shape == (2, 3, 9)
    assert bool((coverage >= 0.0).all())
    assert bool((coverage <= 1.0).all())
    assert bool((remaining >= 0.05 - 1.0e-7).all())
    assert bool((remaining <= 1.0).all())
    assert bool((coverage[:, 1:] + 1.0e-7 >= coverage[:, :-1]).all())
    assert bool((coverage[:, 0].amax(dim=-1) > 0.0).all())


def test_v3_first_relation_step_is_unchanged_by_future_budget() -> None:
    torch.manual_seed(63)
    model = QSREProductionOperatorInducerV3(cfg()).eval()
    q, qmask = query(batch=2, tokens=8, seed=64)
    s = schema(4, seed=65)
    one = run(model, q=q, qmask=qmask, s=s, max_steps=1)
    three = run(model, q=q, qmask=qmask, s=s, max_steps=3)

    assert torch.allclose(
        one["relation_logits"][:, 0],
        three["relation_logits"][:, 0],
        atol=1.0e-6,
        rtol=1.0e-6,
    )
    assert torch.allclose(
        one["event_logits"][:, 0],
        three["event_logits"][:, 0],
        atol=1.0e-6,
        rtol=1.0e-6,
    )


def test_v3_later_steps_see_consumed_query_state() -> None:
    torch.manual_seed(66)
    model = QSREProductionOperatorInducerV3(cfg()).eval()
    q, qmask = query(batch=1, tokens=10, seed=67)
    s = schema(6, seed=68)
    out = run(model, q=q, qmask=qmask, s=s, max_steps=3)

    coverage = out["step_query_coverage"]
    attention = out["step_query_attention"]
    assert float(coverage[0, 0].max()) > 0.0
...
def test_v3_parameter_report_declares_ordered_evidence_runtime_state() -> None:
    report = QSREProductionOperatorInducerV3(cfg()).parameter_report()
    assert report["ordered_query_evidence_consumption"] is True
    assert report["coverage_is_token_position_not_hop_parameter"] is True
    assert report["coverage_preserves_nonzero_evidence_floor"] is True
    assert report["hop_count_dependent_parameters"] == 0
