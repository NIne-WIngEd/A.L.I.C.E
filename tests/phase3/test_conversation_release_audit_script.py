...
SPEC = importlib.util.spec_from_file_location(
    "phase3_release_script",
    ROOT / "scripts" / "run_phase3_conversation_release_audit.py",
)
...
def test_repository_verifier_accepts_clean_exact_commit(tmp_path):
    ...
    assert MODULE._verify_repository_state(
        repo, expected_commit=head, rollback_ref=rollback
    ) == (head, rollback)
...
def test_repository_verifier_rejects_commit_mismatch(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="does not match"):
        MODULE._verify_repository_state(
            repo, expected_commit="0" * 40, rollback_ref=rollback
        )
...
def test_repository_verifier_rejects_same_rollback(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="differ"):
        MODULE._verify_repository_state(
            repo, expected_commit=head, rollback_ref=head
        )


def test_release_parser_generates_evidence_internally():
    destinations={action.dest for action in MODULE._parser()._actions}
    assert "evidence_manifest" in destinations
